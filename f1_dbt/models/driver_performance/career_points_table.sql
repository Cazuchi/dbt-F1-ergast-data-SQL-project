...
FROM {{ ref('driver_adjusted_results_with_bonus') }} arwb
INNER JOIN {{ source('f1', 'drivers') }} d ON arwb.driverid = d.driverid
INNER JOIN {{ source('f1', 'races') }} rc ON arwb.raceid = rc.raceid
CROSS JOIN {{ ref('max_year') }}
